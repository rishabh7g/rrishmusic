...
console.log(
  `📊 Pass Rate: ${Math.round((passingCombinations / totalCombinations) * 100)}%`
)

// Generate recommendations
const recommendations =
  accessibility.generateAccessibilityRecommendations(analysisResults)
...
recommendations.forEach((rec, index) => {
  ...
  if (rec.type === 'SOLUTION' && rec.alternatives) {
    rec.alternatives.forEach(alt => {
      console.log(
        `   • ${alt.name} (${alt.color}): ${Math.round(alt.ratio * 100) / 100}:1 contrast`
      )
    })
  }
  console.log()
