...
import { useCategoryNavigation, useCategoryBreadcrumbs } from '@/hooks/useCategoryNavigation';
...
export const CategoryNavigation: React.FC<CategoryNavigationProps> = ({
  ...
  const breadcrumbs = useCategoryBreadcrumbs(categories, activeCategory, navigationHistory);
  ...
  const renderCategoryButton = (category: CategoryItem, isActive: boolean) => {
    ...
    return (
      <button
        ...
        aria-describedby={showDescriptions && category.description ? `${category.id}-desc` : undefined}
      >
        <div className="flex items-center space-x-2">
          {category.icon && (
            <span className="w-5 h-5" dangerouslySetInnerHTML={{ __html: category.icon }} />
          )}
    ...
    return (
      <div className="category-progress-indicator mt-4">
        <div className="flex justify-between text-xs text-gray-500 mb-1">
          <span>{activeIndex + 1} of {categories.length}</span>
  ...
  const renderBreadcrumbs = () => {
    ...
    return (
      <nav className="category-breadcrumbs mb-4" aria-label="Category breadcrumb">
          ...
          {breadcrumbs.map((crumb, index) => (
            <li key={crumb.id} className="flex items-center">
              ...
              {crumb.isClickable ? (
                ...
                <span className="text-brand-blue-primary font-medium">{crumb.label}</span>
              )}
            </li>
          ))}
        </ol>
  ...
  const renderDesktopNavigation = () => {
    ...
    return (
      <div className={`category-navigation ${containerClasses[variant]} ${className}`}>
        {categories.map(category => renderCategoryButton(
          category, 
  ...
  const renderMobileNavigation = () => {
    ...
    const activeLabel = categories.find(cat => cat.id === activeCategory)?.label || 'Select Category';
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (isMobileMenuOpen && !(event.target as Element).closest('.category-navigation-mobile')) {
        setIsMobileMenuOpen(false);
      }
    };
