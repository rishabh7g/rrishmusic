...
import { TeachingInquiryForm, TeachingInquiryData } from '@/components/forms/TeachingInquiryForm';
...
export const TeachingInquiryCTA: React.FC<TeachingInquiryCTAProps> = ({
  ...
  const handleSubmit = async (data: TeachingInquiryData) => {
    try {
      ...
      alert('Thank you for your inquiry! I\'ll get back to you within 24 hours to discuss your lessons.');
    } catch (error) {
   ...
   */
  const renderCTA = () => {
    const baseClasses = "group transition-all duration-300 focus:outline-none focus:ring-4 focus:ring-brand-orange-warm/20";
    ...
    switch (variant) {
      ...
      case 'prominent':
        return (
          <div className={`bg-brand-orange-warm/10 rounded-2xl p-8 border border-brand-orange-warm/20 ${className}`}>
