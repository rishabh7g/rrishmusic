...
interface SmartContactCTAProps {
   ...
   */
  onFormSubmit?: (serviceType: ServiceType, data: InquiryFormData, context: ContactContext) => void;
...
export const SmartContactCTA: React.FC<SmartContactCTAProps> = ({
  ...
  const handleFormSubmit: FormSubmissionHandler = async (data: InquiryFormData) => {
    ...
    const personalizedMessage = confidenceLevel > 80 
   ...
   */
  const getEnhancedServiceMessage = (context: ContactContext): { 
    title: string; 
    subtitle: string; 
    price?: string; 
    ...
    switch (context.referralSourceType) {
      case 'social_media':
        ...
        break;
      case 'search_engine':
        personalizedNote = 'Great to see you found exactly what you were looking for!';
        break;
      case 'email_campaign':
  ...
  const getEnhancedServiceStyling = (context: ContactContext) => {
    ...
    const confidenceModifier = confidence > 80 ? 'high' : confidence > 50 ? 'medium' : 'low';
  ...
  const renderConfidenceIndicator = (confidence: number) => {
    ...
    return (
      <div className={`inline-flex items-center px-2 py-1 rounded text-xs font-medium ${colors[level]}`}>
  ...
  const renderEnhancedCTA = () => {
    ...
    switch (variant) {
              ...
              <div className="text-center mb-6">
                ...
                {enhancedServiceMessage.price && (
                  <div className="text-white mt-3">
                    <span className="text-3xl font-bold">{enhancedServiceMessage.price}</span>
                  </div>
                )}
                
                {/* Journey context display */}
                {preFillData.userJourneyContext && process.env.NODE_ENV === 'development' && (
      ...
      case 'prominent':
        return (
          <div className={`bg-gradient-to-br ${enhancedServiceStyling.bgGradient} rounded-2xl p-8 text-white ${className}`}>
            {showServiceInfo && (
              <div className="text-center mb-6">
        ...
        return (
          ...
          >
            {showServiceInfo && enhancedServiceMessage.price && (
              <div className="mb-4">
                <div className={`text-${enhancedServiceStyling.color} text-center`}>
                  <span className="text-2xl font-bold">{enhancedServiceMessage.price}</span>
                </div>
