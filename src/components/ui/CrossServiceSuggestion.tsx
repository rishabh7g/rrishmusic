...
export const CrossServiceSuggestion: React.FC<CrossServiceSuggestionProps> = ({
  ...
  const [suggestion, setSuggestion] = useState<CrossServiceSuggestion | null>(null);
  ...
  }, [suggestion, timing, userContext, minTimeOnPage, minScrollPercentage, isDismissed, forceShow]);
  ...
  const renderSuggestion = () => {
    const baseClasses = "bg-gradient-to-br from-gray-50 to-white border border-gray-200 rounded-2xl shadow-sm hover:shadow-md transition-all duration-300";
          ...
          <div className={`${baseClasses} p-4 ${className}`}>
            <div className="flex items-center justify-between">
              <div className="flex-1">
                <p className="text-sm text-gray-600 mb-1">{contextualMessage}</p>
            ...
            <ul className="space-y-2 mb-6">
              {suggestion.benefits.slice(0, 3).map((benefit, index) => (
                <li key={index} className="flex items-start text-sm text-gray-600">
                  <svg className="w-4 h-4 text-brand-blue-primary mr-2 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div className="grid md:grid-cols-2 gap-6 items-center">
              <ul className="space-y-2">
                {suggestion.benefits.map((benefit, index) => (
                  <li key={index} className="flex items-start text-sm text-gray-600">
                    <svg className="w-4 h-4 text-brand-blue-primary mr-3 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                      <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                    </svg>
                    {benefit}
                  </li>
