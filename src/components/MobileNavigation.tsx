 ...
 */
const getIsItemActive = (item: typeof NAVIGATION_ITEMS[0], pathname: string, activeSection: string): boolean => {
...
export const MobileNavigation: React.FC<MobileNavigationProps> = ({
   ...
   */
  const handleNavClick = useCallback(async (item: typeof NAVIGATION_ITEMS[0]) => {
    const target = getNavigationTarget(item);
                ...
                >
                  {NAVIGATION_ITEMS.map((item, index) => (
                    <MobileNavigationItem
                      key={item.id}
                      item={item}
                      isActive={getIsItemActive(item, location.pathname, activeSection)}
