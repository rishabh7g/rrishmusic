...
export const ValidationMessage: React.FC<ValidationMessageProps> = ({
  ...
  const validationError = error || (result && !result.isValid ? {
    field,
    id: `${field}-error`,
    message: result.message,
    severity: result.severity,
    ariaLabel: `${formatFieldName(field)} ${result.severity}: ${result.message}`
  } as AccessibleValidationError : null);
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {/* Suggestions */}
        {showSuggestions && validationError.suggestions && validationError.suggestions.length > 0 && (
          <div className="mt-2">
        ...
        {/* Related Fields */}
        {validationError.relatedFields && validationError.relatedFields.length > 0 && (
          <p className="mt-1 text-xs opacity-60">
            Also check: {validationError.relatedFields.map(formatFieldName).join(', ')}
          </p>
        )}
      </div>
    </div>
...
export const ValidationSummary: React.FC<ValidationSummaryProps> = ({
  ...
  const summaryTitle = title || `Please fix ${errorCount} error${errorCount !== 1 ? 's' : ''}${
  ...
  const groupedErrors = groupByType ? {
    ...
    format: errors.filter(e => !e.message.toLowerCase().includes('required') && 
                          (e.message.toLowerCase().includes('valid') || e.message.toLowerCase().includes('format'))),
    other: errors.filter(e => !e.message.toLowerCase().includes('required') && 
            ...
            <div key={groupName}>
              {groupByType && groupName !== 'all' && groupErrors.length > 0 && (
                <h4 className="text-xs font-medium text-red-700 uppercase tracking-wide mb-2">
                  {groupName === 'required' ? 'Required Fields' : 
                   groupName === 'format' ? 'Format Errors' : 'Other Issues'}
                </h4>
              )}
