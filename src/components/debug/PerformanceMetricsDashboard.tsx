...
export const PerformanceMetricsDashboard: React.FC<PerformanceMetricsDashboardProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'funnels' | 'abtests' | 'optimization'>('overview');
  const [loading, setLoading] = useState(true);
  const [selectedService, setSelectedService] = useState<ServiceType>('teaching');
        ...
        >
          ...
          <div className="bg-gradient-to-r from-brand-blue-primary to-brand-blue-secondary text-white p-6">
            <div className="flex justify-between items-center">
              <div>
                <h2 className="text-2xl font-heading font-bold mb-2">Performance Analytics Dashboard</h2>
                <p className="text-white/80">Comprehensive insights into service performance and optimization</p>
              </div>
              <button
                onClick={onClose}
              ...
              <>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                      ...
                      <div className="bg-gradient-to-br from-blue-50 to-blue-100 p-4 rounded-lg">
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="text-sm font-medium text-blue-800">Active Inquiries</p>
                      ...
                      <div className="bg-gradient-to-br from-purple-50 to-purple-100 p-4 rounded-lg">
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="text-sm font-medium text-purple-800">24h Conversion</p>
                            <p className="text-2xl font-bold text-purple-900">
                              {formatPercentage(dashboardData.realtimeMetrics.conversionRate24h)}
                            </p>
                          </div>
                      ...
                      <div className="bg-gradient-to-br from-amber-50 to-amber-100 p-4 rounded-lg">
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="text-sm font-medium text-amber-800">Today's Revenue</p>
                    ...
                    <div className="bg-white border border-gray-200 rounded-lg p-6">
                      ...
                      <div className="space-y-4">
                        {dashboardData.services.map(service => (
                          <div key={service.service} className="border border-gray-100 rounded-lg p-4">
                            <div className="flex items-center justify-between mb-3">
                              <h4 className={`font-semibold capitalize ${getServiceColor(service.service)}`}>
                                {service.service} Service
                              </h4>
                              <div className={`px-3 py-1 rounded-full text-xs font-medium ${getServiceBgColor(service.service)} ${getServiceColor(service.service)}`}>
                                {formatPercentage(service.conversionFunnel.conversionRate)} conversion
                              </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                              <div>
                                ...
                                <p className="font-semibold">{service.kpis.totalInquiries}</p>
                              </div>
                              <div>
                                <p className="text-gray-500">Avg Revenue</p>
                                <p className="font-semibold">{formatCurrency(service.kpis.averageRevenuePerUser)}</p>
                              </div>
                              <div>
                                <p className="text-gray-500">Quality Score</p>
                                <p className="font-semibold">{service.kpis.serviceQualityScore.toFixed(1)}</p>
                              </div>
                              <div>
                                <p className="text-gray-500">Time to Convert</p>
                                <p className="font-semibold">{service.kpis.timeToConversion} days</p>
                              </div>
                            </div>
                          </div>
                        ))}
                    ...
                    <div className="bg-white border border-gray-200 rounded-lg p-6">
                      <h3 className="text-lg font-semibold mb-4">Cross-Service Performance</h3>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="text-center">
                          <div className="text-2xl font-bold text-brand-blue-primary">
                            {formatPercentage(dashboardData.crossServiceMetrics.crossSellRate)}
                          </div>
                          <p className="text-sm text-gray-600">Cross-sell Rate</p>
                        </div>
                        <div className="text-center">
                          <div className="text-2xl font-bold text-brand-orange-warm">
                            {dashboardData.crossServiceMetrics.averageServicesPerCustomer.toFixed(1)}
                          </div>
                          <p className="text-sm text-gray-600">Avg Services per Customer</p>
                        </div>
                        <div className="text-center">
                          <div className="text-2xl font-bold text-brand-yellow-accent">
                            {dashboardData.crossServiceMetrics.upsellingEffectiveness.toFixed(1)}
                          </div>
                          <p className="text-sm text-gray-600">Upselling Effectiveness</p>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {activeTab === 'funnels' && dashboardData && (
                  <div className="space-y-6">
                    ...
                    <div className="bg-white border border-gray-200 rounded-lg p-6">
                      {(() => {
                        const serviceData = dashboardData.services.find(s => s.service === selectedService);
                        if (!serviceData) return <p>No data available</p>;

                        return (
                          <div className="space-y-4">
                            <div className="flex items-center justify-between">
                              <h4 className={`font-semibold capitalize ${getServiceColor(selectedService)}`}>
                                {selectedService} Conversion Funnel
                              </h4>
                              <div className="text-sm text-gray-600">
                                Total Users: {serviceData.conversionFunnel.totalUsers}
                              </div>
                            </div>

                            {/* Funnel Visualization */}
                            <div className="space-y-3">
                              {serviceData.conversionFunnel.stages.map((stage, index) => (
                                    ...
                                    <div className="flex-1">
                                      <div className="flex items-center justify-between mb-1">
                                        ...
                                        <div className="flex items-center space-x-4 text-sm">
                                          ...
                                          {index > 0 && (
                                            <span className={stage.conversionFromPrevious < 70 ? 'text-red-600' : 'text-green-600'}>
                                              {formatPercentage(stage.conversionFromPrevious)} conversion
                                            </span>
                                          )}
                                        </div>
                                      </div>
                                      ...
                                      {stage.dropoffRate > 30 && (
                                        <p className="text-xs text-red-600 mt-1">
                                          High dropoff: {formatPercentage(stage.dropoffRate)}
                                        </p>
                                      )}
                                    </div>
                                  </div>
                            ...
                            {/* Dropoff Analysis */}
                            {serviceData.conversionFunnel.dropoffAnalysis.length > 0 && (
                              <div className="mt-6 p-4 bg-red-50 rounded-lg">
                                <h5 className="font-semibold text-red-800 mb-2">Dropoff Analysis</h5>
                                <div className="space-y-2">
                                  {serviceData.conversionFunnel.dropoffAnalysis.map(dropoff => (
                                    <div key={dropoff.stageName} className="text-sm">
                                      <div className="font-medium text-red-700 capitalize">
                                        {dropoff.stageName.replace('_', ' ')}: {formatPercentage(dropoff.dropoffRate)} dropoff
                                      </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="bg-white border border-gray-200 rounded-lg p-6">
                      <div className="text-center py-8">
                        ...
                        <h4 className="text-lg font-semibold mb-2">A/B Testing Framework Ready</h4>
                        <p className="text-gray-600 mb-4">
                          Advanced A/B testing system is implemented and ready for experiments.
                        </p>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-md mx-auto">
                          <div className="p-4 border border-gray-200 rounded-lg">
                            <div className="text-2xl font-bold text-brand-blue-primary">95%</div>
                            <div className="text-sm text-gray-600">Statistical Confidence</div>
                          </div>
                          <div className="p-4 border border-gray-200 rounded-lg">
                            <div className="text-2xl font-bold text-brand-orange-warm">Auto</div>
                ...
                {activeTab === 'optimization' && dashboardData && (
                  <div className="space-y-6">
                      ...
                      {dashboardData.optimizationOpportunities.map((opportunity, index) => (
                        ...
                        >
                          <div className="flex items-start justify-between mb-3">
                            <div className="flex-1">
                              <h4 className="font-semibold text-gray-900 mb-1">{opportunity.area}</h4>
                          ...
                          <div className="mb-3">
                            ...
                            <div className="text-sm text-gray-700">{opportunity.expectedImprovement}</div>
                          </div>
                          
                          <div>
                            <div className="text-sm font-medium text-gray-800 mb-2">Implementation Steps:</div>
                            <ul className="list-disc list-inside space-y-1 text-sm text-gray-600">
                              {opportunity.implementationSteps.map((step, stepIndex) => (
                    ...
                    {dashboardData.optimizationOpportunities.length === 0 && (
                      <div className="text-center py-8">
                        <div className="text-4xl mb-4">🎯</div>
                        <h4 className="text-lg font-semibold mb-2">All Systems Optimized</h4>
                        <p className="text-gray-600">
                          No immediate optimization opportunities detected. Keep monitoring for improvements.
                        </p>
                      </div>
                    )}
                  </div>
