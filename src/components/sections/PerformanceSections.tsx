  ...
  const venues = [
    {
      ...
      description: 'Main stage performance at Melbourne\'s premier blues festival',
      image: '/api/placeholder/400/300',
      year: '2024',
      highlights: ['Main stage slot', 'Audience of 5000+', 'Live recording released']
    },
    {
      id: 2,
    ...
    },
    {
      ...
      capacity: '50-500',
      location: 'Various',
      description: 'Professional entertainment for corporate functions and private events',
      image: '/api/placeholder/400/300',
      year: 'Ongoing',
      highlights: ['High-profile clients', 'Custom setlists', 'Professional setup']
    }
  ];
  ...
  return (
    ...
    >
      <div className="venues-grid grid md:grid-cols-2 gap-8">
        {venues.map((venue) => (
          <div key={venue.id} className="venue-card bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300">
...
export const PerformanceServices: React.FC<ServiceSectionProps> = ({ 
  ...
  const services = {
    venue: {
      ...
      description: 'Professional live music for bars, restaurants, and music venues',
      features: [
    ...
    private: {
      ...
      description: 'Intimate acoustic performances for weddings, parties, and celebrations',
      features: [
    ...
    corporate: {
      ...
      description: 'Sophisticated entertainment for business events and conferences',
      features: [
    ...
    session: {
      title: 'Session Work',
      description: 'Professional guitar services for recordings, tours, and projects',
      features: [
...
export const PerformanceEquipment: React.FC<ServiceSectionProps> = ({ 
  ...
  const equipment = {
    guitars: [
      { name: 'Fender Stratocaster', type: 'Electric', use: 'Blues, Rock, Jazz' },
      { name: 'Gibson Les Paul', type: 'Electric', use: 'Blues, Rock' },
      { name: 'Martin Acoustic', type: 'Acoustic', use: 'Folk, Unplugged' }
    ],
    amplification: [
      { name: 'Fender Blues Deluxe', power: '40W Tube', use: 'Small-Medium Venues' },
      ...
      <div className="equipment-sections grid lg:grid-cols-3 gap-8">
        ...
        <div className="equipment-category bg-white rounded-2xl p-6 shadow-lg">
          ...
          <div className="equipment-list space-y-4">
            {equipment.guitars.map((guitar, index) => (
              <div key={index} className="equipment-item p-3 bg-blue-50 rounded-lg">
                <div className="font-medium text-gray-900">{guitar.name}</div>
                <div className="text-sm text-gray-600">{guitar.type}</div>
                <div className="text-xs text-brand-blue-primary">{guitar.use}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="equipment-category bg-white rounded-2xl p-6 shadow-lg">
          <div className="category-header text-center mb-6">
            ...
            <h3 className="text-xl font-bold text-gray-900">Effects & Pedals</h3>
          </div>
          
          <div className="equipment-list space-y-2">
            {equipment.effects.map((effect, index) => (
              <div key={index} className="equipment-item p-3 bg-blue-50 rounded-lg text-center">
                <div className="font-medium text-gray-900 text-sm">{effect}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="setup-info mt-12 bg-white rounded-2xl p-8 shadow-lg">
        ...
        <div className="grid md:grid-cols-3 gap-6 text-center">
          <div className="setup-feature">
            ...
            <h4 className="font-semibold text-gray-900 mb-2">Sound Engineering</h4>
            <p className="text-sm text-gray-600">Professional sound mixing and level control</p>
          </div>
          <div className="setup-feature">
            <div className="w-12 h-12 bg-brand-blue-primary text-white rounded-full flex items-center justify-center mx-auto mb-3">
              📡
            </div>
            <h4 className="font-semibold text-gray-900 mb-2">Wireless Freedom</h4>
            <p className="text-sm text-gray-600">Wireless guitar system for stage mobility</p>
          </div>
          <div className="setup-feature">
            <div className="w-12 h-12 bg-brand-blue-primary text-white rounded-full flex items-center justify-center mx-auto mb-3">
              🔧
            ...
            <p className="text-sm text-gray-600">Efficient setup and soundcheck process</p>
          </div>
        </div>
      </div>
    </ServiceSectionTemplate>
  ...
  const testimonials = [
    {
      ...
      role: 'Venue Manager',
      quote: 'Rrish brings exactly what we look for - professional attitude, great sound, and the ability to read and engage with the crowd. He\'s become one of our regular artists.',
      rating: 5,
    ...
    },
    {
      ...
      quote: 'Reliable, professional, and musically excellent. Rrish has performed at multiple corporate functions for us and always delivers exactly what the client needs.',
      rating: 5,
    ...
    },
    {
      ...
      role: 'Bride',
      quote: 'Our wedding ceremony was perfect thanks to Rrish\'s beautiful acoustic performance. He created the exact atmosphere we wanted for our special day.',
      rating: 5,
    ...
    >
      <div className="testimonials-grid grid md:grid-cols-3 gap-8">
        {testimonials.map((testimonial) => (
          <div key={testimonial.id} className="testimonial-card bg-gradient-to-br from-blue-50 to-white rounded-2xl p-6 shadow-lg border-l-4 border-brand-blue-primary">
            <div className="testimonial-header mb-4">
              ...
              <div className="text-xs text-brand-blue-primary font-medium">{testimonial.event}</div>
            </div>
