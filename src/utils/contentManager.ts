...
export const useContentCalculations = () => {
  ...
  const calculateTestimonialStats = useCallback(
    async (testimonials: Testimonial[]): Promise<TestimonialStats> => {
  ...
  const calculateLessonPackagePricing = useCallback(
    async (lessonPackage: LessonPackage): Promise<Record<string, unknown>> => {
...
export async function calculateTestimonialStats(
...
export async function calculatePerformanceData(
  rawData: Record<string, unknown>
): Promise<EnhancedPerformanceData> {
  return dataCalculator.calculatePerformanceData(rawData)
}
...
export async function calculateLessonPackagePricing(
...
export async function batchCalculations<T extends Record<string, unknown>>(
  ...
  await Promise.allSettled(
    Object.entries(calculations).map(async ([key, calculationFn]) => {
      try {
        results[key as keyof T] = await (
          calculationFn as () => Promise<T[keyof T]>
        )()
      } catch (error) {
