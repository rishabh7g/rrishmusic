...
const BUILT_IN_VALIDATORS = {
  ...
  confirmPassword: (value: unknown, options: { originalPassword: string }): boolean => {
 ...
 */
const SERVICE_FIELD_REQUIREMENTS: Record<ServiceType, ServiceFieldRequirements> = {
  teaching: {
        ...
        rules: [
          {
            ...
            field: 'experience',
            validator: (value) => BUILT_IN_VALIDATORS.pattern(value, { 
        ...
        rules: [
          {
            ...
            validator: (value) => BUILT_IN_VALIDATORS.minLength(value, { min: 20 }),
...
export function getServiceValidationRules(
  ...
  requirements.required.forEach(field => {
    rules.push(createValidationRule(field, 'required', {}, { isRequired: true }));
  ...
  if (requirements.required.includes('email') || requirements.optional.includes('email')) {
    rules.push(createValidationRule('email', 'email'));
  }

  if (requirements.required.includes('phone') || requirements.optional.includes('phone')) {
...
export function validateField(
  ...
  const fieldRules = rules.filter(rule => 
    rule.field === field && (!rule.when || rule.when(context))
  );
...
function getSuggestions(
  ...
  if (field === 'phone' && result.message.includes('valid phone')) {
    suggestions.push('Include area code', 'Format: (555) 123-4567 or +1-555-123-4567');
  }

  if (serviceType === 'teaching' && field === 'goals') {
    suggestions.push('Example: "I want to learn acoustic guitar and play my favorite songs"');
  }

  if (serviceType === 'performance' && field === 'venue') {
    suggestions.push('Include venue name and city', 'Example: "Central Park, New York"');
  }
 ...
 */
export function getFieldValidationPriority(field: string, serviceType: ServiceType): number {
  const requirements = SERVICE_FIELD_REQUIREMENTS[serviceType];
