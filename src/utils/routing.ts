...
export const navigateTo = (path: string): void => {
  ...
  if (!isValidRoute(cleanPath)) {
    console.warn(
      `[Routing] Invalid route attempted: ${cleanPath}. Redirecting to homepage.`
    )
  ...
  if (search && search[1] === '/') {
    try {
      ...
      if (isValidRoute(originalPath)) {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log(
            '[SPA Redirect] Successfully restored route:',
            originalPath
          )
        }
      } else {
...
export const initializeRouting = (): void => {
  ...
  const handlePopState = () => {
    ...
    if (!isValidRoute(currentPath)) {
      console.warn(
        `[Routing] Invalid route detected: ${currentPath}. Redirecting to homepage.`
      )
      navigateTo('/')
    }
  }
