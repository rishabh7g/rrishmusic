...
export function analyzeYellowAccentAccessibility() {
  ...
  for (const [bgName, bgColor] of Object.entries(backgroundColors)) {
    ...
    console.log(
      `WCAG AA Normal: ${compliance.AA_normal ? '✅ PASS' : '❌ FAIL'} (≥4.5:1)`
    )
    console.log(
      `WCAG AA Large:  ${compliance.AA_large ? '✅ PASS' : '❌ FAIL'} (≥3.0:1)`
    )
    console.log(
      `WCAG AAA Normal:${compliance.AAA_normal ? '✅ PASS' : '❌ FAIL'} (≥7.0:1)`
    )
...
export function generateAccessibilityRecommendations(
  results: ReturnType<typeof analyzeYellowAccentAccessibility>
) {
  ...
  if (failedCombinations.length === 0) {
    recommendations.push({
      ...
      priority: 'LOW',
      message:
        '🎉 All yellow accent color combinations meet WCAG AA standards!',
    })
      ...
      alternatives: [
        {
          color: '#f59e0b',
          name: 'Darker Yellow',
          ratio: getContrastRatio('#f59e0b', brandColors['brand-blue-primary']),
        },
        {
          color: '#d97706',
          name: 'Amber Orange',
          ratio: getContrastRatio('#d97706', brandColors['brand-blue-primary']),
        },
        {
          color: '#ffffff',
          name: 'White Text',
          ratio: getContrastRatio('#ffffff', brandColors['brand-blue-primary']),
        },
      ],
    })
  }
 ...
 */
export function suggestImprovedYellowAccent(
  targetBackgrounds: string[] = [brandColors['brand-blue-primary']]
) {
  const suggestions = []
  ...
  for (const variation of yellowVariations) {
    ...
    console.log(
      `${variation.name.padEnd(15)} | ${variation.hex} | ${allPass ? '✅ RECOMMENDED' : '❌ NOT SUITABLE'}`
    )
    results.forEach(r => {
      const bgName =
        Object.entries(brandColors).find(([, v]) => v === r.background)?.[0] ||
        r.background
