...
function calculateTestimonialStats() {
  ...
  const testimonialsByService = testimonials.reduce(
    (acc, testimonial) => {
  ...
  const averageRating =
    testimonials.length > 0 ? ratingsSum / testimonials.length : 4.9

  // Count unique locations (venues)
  const uniqueLocations = new Set(
    testimonials.map(t => t.location).filter(loc => loc && loc !== '')
  )

  // Calculate performance-specific stats
  const performanceTestimonials = testimonials.filter(
    t => t.service === 'performance'
  )
  const weddingCount = performanceTestimonials.filter(
    t => t.serviceSubType === 'wedding'
  ).length
  const corporateCount = performanceTestimonials.filter(
    t => t.serviceSubType === 'corporate'
  ).length
  const privateCount = performanceTestimonials.filter(
    t =>
      t.serviceSubType && !['wedding', 'corporate'].includes(t.serviceSubType)
  ).length

  // Calculate collaboration stats
  const collaborationTestimonials = testimonials.filter(
    t => t.service === 'collaboration'
  )
...
function calculateLessonStats() {
  ...
  const estimatedTotalStudents = Math.max(
    45,
    Math.floor(estimatedActiveStudents * 1.6)
  )
  const estimatedCompletedStudents =
    estimatedTotalStudents - estimatedActiveStudents
  ...
  const averageProgress = Math.min(
    95,
    70 + (packageComplexity / totalPackages) * 2
  )
...
function calculatePerformanceMetrics(
  ...
  const weddingPerformances = Math.max(
    40,
    performanceStats.wedding * scaleFactor
  )
...
function calculateQualityMetrics(
  ...
  const completionRate = Math.min(
    100,
    Math.max(92, Math.floor(averageProgress * 1.08))
  )
