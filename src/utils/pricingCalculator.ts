  ...
  packages: [
    {
      ...
      duration: 30,
      validity: 30,
      features: ['30-minute session', 'Teaching style introduction', 'Goal assessment']
    },
    {
      id: 'single',
      sessions: 1,
      duration: 60,
      validity: 60,
      features: ['Full 60-minute lesson', 'Personalized instruction', 'Practice materials']
    },
    {
      id: 'foundation',
      sessions: 4,
      duration: 60,
      validity: 90,
      features: ['4 weekly lessons', '5% bulk discount', 'Progress tracking', 'Email support']
    },
    {
      id: 'transformation',
      sessions: 8,
      ...
      validity: 120,
      popular: true,
      features: ['8 weekly lessons', '10% bulk discount', 'Comprehensive curriculum', 'Performance preparation']
    }
  ]
};
...
export function calculatePackagePricing(
  ...
  const basePrice = sessions === 1 && sessions < bulkDiscountThreshold 
...
export function getPackageWithPricing(
  ...
  const packageConfig = TEACHING_PRICING_CONFIG.packages.find(pkg => pkg.id === packageId);
...
function calculateValueScore(
  ...
  const efficiencyScore = Math.max(0, (TEACHING_PRICING_CONFIG.baseLessonPrice - pricing.pricePerLesson));
...
function getPackageDescription(
  ...
  let description = baseDescriptions[sessions] || `${sessions} personalized guitar lessons`;
