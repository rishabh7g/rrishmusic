...
export class UpsellingAnalytics {
  ...
  trackEvent(event: Omit<UpsellingEvent, 'eventId' | 'timestamp'>): void {
    ...
    try {
      const storedEvents = JSON.parse(
        localStorage.getItem('rrish_upselling_events') || '[]'
      )
   ...
   */
  trackConversion(
    conversion: Omit<CrossServiceConversion, 'conversionId' | 'timestamp'>
  ): void {
   ...
   */
  getCrossServiceConversionRate(
    fromService: ServiceType,
    toService: ServiceType
  ): number {
    const totalInquiries = this.events.filter(
      e => e.eventType === 'service_inquiry' && e.fromService === fromService
    ).length

    const conversions = this.conversions.filter(
      c => c.originalService === fromService && c.convertedService === toService
    ).length
  ...
  calculateUpsellingROI(timeframe: number = 30): number {
    ...
    const totalValue = recentConversions.reduce(
  ...
  generateOptimizationRecommendations(): string[] {
    ...
    if (lowCTR.length > 0) {
      recommendations.push(
        `${lowCTR.length} suggestions have low click-through rates. Consider improving headlines or placement.`
      )
    }
    ...
    if (highDismissal.length > 0) {
      recommendations.push(
        `${highDismissal.length} suggestions have high dismissal rates. Review targeting and timing.`
      )
    }
    ...
    if (lowConversion.length > 0) {
      recommendations.push(
        `${lowConversion.length} suggestions have low conversion rates. Consider A/B testing different approaches.`
      )
    }
    ...
    Object.entries(serviceRates).forEach(([fromTo, rate]) => {
      if (rate < 5) {
        recommendations.push(
          `${fromTo} conversion rate is ${rate}%. Consider new suggestion strategies.`
        )
      }
    })
  ...
  private updateMetrics(event: UpsellingEvent): void {
    ...
    existing.clickThroughRate =
      existing.impressions > 0
        ? (existing.clicks / existing.impressions) * 100
        : 0

    existing.conversionRate =
      existing.clicks > 0 ? (existing.conversions / existing.clicks) * 100 : 0

    existing.dismissalRate =
      existing.impressions > 0
        ? (existing.dismissals / existing.impressions) * 100
        : 0
...
export function useUpsellingAnalytics() {
  const trackSuggestionView = (
    suggestionId: string,
    context: UpsellingEvent['context']
  ) => {
    upsellingAnalytics.trackEvent({
      suggestionId,
      sessionId: upsellingAnalytics['getSessionId'](),
      eventType: 'suggestion_viewed',
      fromService: context.pageSection.includes('teaching')
        ? 'teaching'
        : context.pageSection.includes('performance')
          ? 'performance'
          : 'collaboration',
      context,
    })
  ...
  return {
    ...
    getTopPerforming:
      upsellingAnalytics.getTopPerformingSuggestions.bind(upsellingAnalytics),
    getRecommendations:
      upsellingAnalytics.generateOptimizationRecommendations.bind(
        upsellingAnalytics
      ),
  }
}
