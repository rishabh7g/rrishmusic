 ...
 */
export type RecoveryStrategy = 'retry' | 'navigate-home' | 'safe-refresh' | 'report';
...
export class ErrorRecoveryManager {
  ...
  public async recoverFromError(
    ...
    console.log(`[ErrorRecovery] Attempting recovery with strategy: ${strategy}`, context);
    ...
    try {
      switch (strategy) {
        ...
        default:
          console.warn(`[ErrorRecovery] Unknown strategy: ${strategy}, falling back to navigate-home`);
          return await this.navigateToHomepage();
      }
    } catch (recoveryError) {
  ...
  private async retryOperation(context: RecoveryContext): Promise<boolean> {
    if (context.retryCount >= this.maxRetries) {
      console.warn('[ErrorRecovery] Max retries exceeded, switching to navigation strategy');
      return await this.navigateToHomepage();
    }

    const delay = this.retryDelay * Math.pow(2, context.retryCount);
    console.log(`[ErrorRecovery] Retrying in ${delay}ms (attempt ${context.retryCount + 1}/${this.maxRetries})`);
  ...
  private async navigateToHomepage(): Promise<boolean> {
    try {
      ...
      console.error('[ErrorRecovery] Homepage navigation failed:', navigationError);
  ...
  private async performSafeRefresh(): Promise<boolean> {
    try {
      ...
      if (isValidRoute(currentPath)) {
        console.log(`[ErrorRecovery] Safe refresh of valid route: ${currentPath}`);
        window.location.reload();
        return true;
      } else {
        console.warn(`[ErrorRecovery] Current route ${currentPath} is not safe to refresh, navigating to homepage`);
        return await this.navigateToHomepage();
      }
    } catch (refreshError) {
  ...
  private async reportError(context: RecoveryContext): Promise<boolean> {
    try {
      ...
      const subject = encodeURIComponent(`RrishMusic Error Report - ${context.error.name}`);
...
Generated by Error Recovery System
      ...
      alert(`An error occurred on RrishMusic. Please contact hello@rrishmusic.com with the following details:\n\nError: ${context.error.message}\nURL: ${context.url}\nTime: ${context.timestamp}`);
   ...
   */
  public getRecommendedStrategy(error: Error, retryCount = 0): RecoveryStrategy {
...
export function useErrorRecovery() {
  ...
  return {
    ...
    logError: (error: Error, strategy: RecoveryStrategy = 'retry', retryCount = 0) => 
      manager.logError({
        error,
        url: window.location.href,
        retryCount,
