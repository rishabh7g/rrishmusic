...
export function estimatePerformancePricing(data: {
  ...
  // Get base rate
  basePrice = PERFORMANCE_PRICING_CONFIG.baseRates[format as 'solo' | 'band'][style as 'acoustic' | 'electric' | 'both'];
  
  // Event type adjustment
  const eventMultiplier = PERFORMANCE_PRICING_CONFIG.eventTypeMultipliers[data.eventType];
  ...
  const durationMultiplier = PERFORMANCE_PRICING_CONFIG.durationAdjustments[durationKey];
  ...
  if (data.guestCount) {
    ...
    const guestMultiplier = PERFORMANCE_PRICING_CONFIG.guestCountAdjustments[guestKey];
  ...
  if (data.eventDate) {
    ...
    const seasonMultiplier = PERFORMANCE_PRICING_CONFIG.seasonalAdjustments[season];
  ...
  const confidence = getEstimateConfidence([
    data.performanceFormat !== 'unsure' && data.performanceFormat !== 'flexible',
  ...
  return {
    ...
    confidence,
    factors,
    consultationRecommended: confidence === 'low' || data.budgetRange === 'discuss',
...
export function estimateCollaborationPricing(data: {
  ...
  const scopeMultiplier = COLLABORATION_PRICING_CONFIG.scopeMultipliers[data.projectScope];
  factors.push(`Project scope: ${data.projectScope}`);
  
  // Timeline adjustment
  const timelineAdjustment = COLLABORATION_PRICING_CONFIG.timelineAdjustments[data.timeline];
  ...
  const experienceAdjustment = COLLABORATION_PRICING_CONFIG.experienceAdjustments[data.experience];
  ...
  const complexityAdjustment = COLLABORATION_PRICING_CONFIG.complexityFactors[complexity];
  ...
  return {
    ...
    consultationRecommended: confidence === 'low' || complexity === 'expert' || data.budgetRange === 'discuss',
...
function assessProjectComplexity(creativeVision: string): 'simple' | 'moderate' | 'complex' | 'expert' {
...
function getProjectHours(scope: string, complexity: string): number {
  ...
  return Math.round(baseHours[scope as keyof typeof baseHours] * complexityMultipliers[complexity as keyof typeof complexityMultipliers]);
}
