...
export class PaymentProcessor {
  ...
  getAvailablePaymentMethods(
    ...
    for (const gateway of this.gateways.values()) {
      ...
      if (serviceType === 'teaching' && amount >= 10000) {
        // $100+
  ...
  calculateFees(
    ...
    const processingFee = Math.round(
      (amount * gatewayConfig.fees.percentage) / 100 + gatewayConfig.fees.fixed
    )
  ...
  private validatePaymentRequest(request: PaymentRequest): PaymentError | null {
    ...
    if (
      ...
      !request.paymentMethod.cardToken
    ) {
      return {
  ...
  private selectPaymentGateway(request: PaymentRequest): PaymentGateway | null {
    for (const gatewayName of this.config.enabledGateways) {
      ...
      const supportsMethod = gateway.supportedMethods.some(
        method =>
          method.type === request.paymentMethod.type &&
          method.isEnabled &&
          (!request.paymentMethod.subtype ||
            method.subtype === request.paymentMethod.subtype)
      )
...
export const DEFAULT_PAYMENT_CONFIG: PaymentConfig = {
  ...
  square: {
    applicationId:
      process.env.REACT_APP_SQUARE_APPLICATION_ID || 'square_app_id',
