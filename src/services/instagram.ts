...
const FALLBACK_INSTAGRAM_POSTS: InstagramPost[] = [
  {
    id: 'fallback-1',
    caption:
      'Blues session at Melbourne venue last weekend. Great crowd and amazing energy! 🎸 #livemusic #blues #melbourne',
  ...
  },
  {
    id: 'fallback-2',
    caption:
      'Acoustic set for a beautiful wedding ceremony. Congratulations to the happy couple! 💍🎵 #weddingmusic #acoustic #liveperformance',
  ...
  },
  {
    id: 'fallback-3',
    caption:
      'Corporate event entertainment in Melbourne CBD. Music creates the perfect networking atmosphere. 🏢🎶 #corporate #livemusic #networking',
  ...
  },
  {
    id: 'fallback-4',
    caption:
      "Behind the scenes: Setting up for tonight's performance. The calm before the musical storm! 🎸⚡ #behindthescenes #setup #preparation",
...
class InstagramService {
  private cache: Map<string, { data: InstagramPost[]; timestamp: number }> =
    new Map()
  ...
  private async fetchFromAPI(limit: number = 12): Promise<InstagramPost[]> {
    ...
    const fields =
      'id,caption,media_url,media_type,permalink,timestamp,thumbnail_url,username'
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : 'Failed to load Instagram content from JSON',
      }
    }
  }
