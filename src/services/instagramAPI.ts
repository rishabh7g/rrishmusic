...
export class InstagramAPIService {
  ...
  constructor(config?: Partial<InstagramAPIConfig>) {
    this.config = {
      accessToken: process.env.REACT_APP_INSTAGRAM_ACCESS_TOKEN || config?.accessToken,
      clientId: process.env.REACT_APP_INSTAGRAM_CLIENT_ID || config?.clientId,
      clientSecret: process.env.REACT_APP_INSTAGRAM_CLIENT_SECRET || config?.clientSecret,
  ...
  private saveRateLimitState(): void {
    try {
      localStorage.setItem('instagram_rate_limit', JSON.stringify({
        ...this.rateLimit,
        timestamp: Date.now()
      }));
    } catch {
  ...
  private async makeRequest<T = Record<string, unknown>>(
    ...
    if (!this.config.accessToken) {
      throw new InstagramAPIError('Instagram access token not configured', 401, 'auth');
    }
  ...
  async getMediaDetails(mediaId: string): Promise<InstagramMediaDetails> {
    return this.makeRequest<InstagramMediaDetails>(`${mediaId}`, {
      fields: 'id,caption,media_url,media_type,permalink,timestamp,thumbnail_url,username,like_count,comments_count,media_product_type,video_title,children{media_url,media_type}'
    });
  }
  ...
  async getUserMedia(options: {
    ...
    const params: Record<string, unknown> = {
      fields: 'id,caption,media_url,media_type,permalink,timestamp,thumbnail_url,username,like_count,comments_count',
  ...
  async getMediaInsights(mediaId: string): Promise<InstagramAPIMetrics> {
    try {
      const insights = await this.makeRequest<InsightsResponse>(`${mediaId}/insights`, {
  ...
  async refreshToken(): Promise<{ access_token: string; expires_in: number }> {
    ...
    return this.makeRequest<{ access_token: string; expires_in: number }>('refresh_access_token', {
   ...
   */
  async validateToken(): Promise<{ is_valid: boolean; expires_at?: number; scopes?: string[] }> {
    try {
  ...
  getRateLimitStatus(): {
    ...
    return {
      ...
      requestsRemaining: Math.max(0, this.config.rateLimit.requestsPerHour - this.rateLimit.requests),
      resetTime: this.rateLimit.resetTime,
      burstRequests: this.rateLimit.burstRequests,
      burstRemaining: Math.max(0, this.config.rateLimit.burstLimit - this.rateLimit.burstRequests),
