...
export interface UseInquiryPricingReturn {
  ...
  actions: {
    ...
    scheduleConsultation: (
      booking: Omit<ConsultationBooking, 'id' | 'status'>
    ) => void
...
export function useInquiryPricing(
   ...
   */
  const estimatePerformancePrice = useCallback(
    async (data: PerformancePricingData) => {
   ...
   */
  const estimateCollaborationPrice = useCallback(
    async (data: CollaborationPricingData) => {
      setState(prev => ({ ...prev, isEstimating: true, error: null }))
  ...
  const computed = useMemo(() => {
    ...
    const isConsultationRecommended = Boolean(
      state.currentEstimate?.consultationRecommended
    )
    const formattedEstimate = state.currentEstimate
      ? formatPriceEstimate(state.currentEstimate)
      : null
    ...
    const estimateExpired = Boolean(
      state.currentEstimate &&
        Date.now() >
          Date.now() +
            state.currentEstimate.estimateValidDays * 24 * 60 * 60 * 1000
    )

    const canBook =
      enableConsultationBooking && hasEstimate && !state.consultationBooking
...
export function useSimpleInquiryPricing(serviceType: ServiceType) {
  const [currentEstimate, setCurrentEstimate] = useState<PriceEstimate | null>(
    null
  )
  const [isEstimating, setIsEstimating] = useState(false)

  const estimatePrice = useCallback(
    async (data: PerformancePricingData | CollaborationPricingData) => {
      ...
      try {
        ...
        if (serviceType === 'performance') {
          ...
          estimate = estimateCollaborationPricing(
            data as CollaborationPricingData
          )
        }
  ...
  return {
    ...
    estimatePrice,
    clearEstimate,
    formattedEstimate: currentEstimate
      ? formatPriceEstimate(currentEstimate)
      : null,
  }
}
...
export const InquiryPricingUtils = {
   ...
   */
  validateInquiryData: (
    data: PerformancePricingData | CollaborationPricingData
  ): {
    isValid: boolean
    ...
    const providedOptional = optionalFields.filter(
      field =>
    ...
    const completeness = Math.round(
      (((requiredFields.length - missingRequired.length) /
        ...
        providedOptional.length / optionalFields.length) /
        2) *
        100
    )
