 ...
 */
export function useServiceContent(service: ServiceType): UseServiceContentResult {
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  // Load content based on service type
  const { data: lessonsData, loading: lessonsLoading, error: lessonsError } = useSectionContent('lessons');
  const { data: performanceData, loading: performanceLoading, error: performanceError } = useSectionContent('performance');
    ...
    try {
      if (service === 'teaching') {
        ...
        return {
          ...
          pricing: {
            from: typedLessonsData.packages?.reduce((min: number, pkg: LessonPackage) => 
              Math.min(min, pkg.price || Infinity), Infinity) || 50,
      ...
      if (service === 'collaboration') {
        const collaborationTestimonials = getTestimonialsByService('collaboration');
  ...
  }, [service, lessonsData, performanceData, getTestimonialsByService, getServiceStats]);

  // Update loading and error states
  useEffect(() => {
    const isLoading = lessonsLoading || performanceLoading || testimonialsLoading;
    ...
    lessonsError, performanceError, testimonialsError
  ]);
