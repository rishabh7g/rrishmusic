...
export const useCategoryNavigation = (
  ...
  const { scrollToCategory, scrollToPreviousPosition } =
    useCategorySmoothScroll(categories)
    ...
    (categoryId: string) => {
      ...
      if (scrollState.activeCategory !== categoryId) {
        setNavigationHistory(prev =>
          [...prev, scrollState.activeCategory].slice(-10)
        ) // Keep last 10
      }

      // Restore scroll position if available
      const savedPosition = persistScrollPosition
        ? scrollPositions[categoryId]
        : null
  ...
  const navigateNext = useCallback(() => {
    const currentIndex = categories.findIndex(
      cat => cat.id === scrollState.activeCategory
    )
...
export const useCategoryBreadcrumbs = (
  ...
  return useMemo(() => {
    ...
    for (const categoryId of navigationHistory.slice(-3)) {
      // Show last 3
