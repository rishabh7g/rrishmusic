...
export function useContextualValidation(
  ...
  const progressiveStages = useMemo(() => {
    ...
    const defaultStages = {
      teaching: [
        { name: 'contact', fields: ['name', 'email'], trigger: 'blur' as const },
        { name: 'experience', fields: ['experience', 'goals'], trigger: 'change' as const },
        { name: 'preferences', fields: ['instrument', 'preferredTime'], trigger: 'submit' as const }
      ],
      performance: [
        { name: 'contact', fields: ['name', 'email'], trigger: 'blur' as const },
        { name: 'event', fields: ['eventType', 'eventDate', 'venue'], trigger: 'change' as const },
        { name: 'details', fields: ['duration', 'guestCount', 'specialRequests'], trigger: 'submit' as const }
      ],
      collaboration: [
        { name: 'contact', fields: ['name', 'email'], trigger: 'blur' as const },
        { name: 'project', fields: ['projectType', 'projectDescription'], trigger: 'change' as const },
        { name: 'details', fields: ['genre', 'timeline', 'budget'], trigger: 'submit' as const }
      ]
    };
  ...
  const validateFieldWithContext = useCallback((
    ...
    if (!immediate && !shouldValidateField(field, currentStage, progressiveStages)) {
      return {
        field,
        isValid: true,
        severity: 'info',
    ...
    const result = validateField(field, value, validationRules, validationContext);
    ...
    return result;
  }, [validationRules, validationContext, currentStage, progressiveStages, onFieldValidation]);

  /**
   * Validate all form fields
   */
  const validateAllFields = useCallback((immediate = false) => {
    setValidationState(prev => ({
      ...prev,
      isValidating: true
    }));

    const results = validateForm(debouncedFormData, serviceType, validationContext);
    ...
    const hasErrors = Object.values(filteredResults).some(r => !r.isValid && r.severity === 'error');
    const hasWarnings = Object.values(filteredResults).some(r => !r.isValid && r.severity === 'warning');
  ...
  const validateSingleField = useCallback((field: string, value?: unknown) => {
    ...
    setValidationState(prev => ({
      ...
      hasErrors: Object.values({ ...prev.fieldResults, [field]: result })
        .some(r => !r.isValid && r.severity === 'error'),
      hasWarnings: Object.values({ ...prev.fieldResults, [field]: result })
        .some(r => !r.isValid && r.severity === 'warning'),
      isValid: !Object.values({ ...prev.fieldResults, [field]: result })
        .some(r => !r.isValid && r.severity === 'error')
    }));
  ...
  const clearFieldValidation = useCallback((field: string) => {
    setValidationState(prev => {
      ...
      return {
        ...
        touchedFields: newTouchedFields,
        hasErrors: Object.values(newFieldResults).some(r => !r.isValid && r.severity === 'error'),
        hasWarnings: Object.values(newFieldResults).some(r => !r.isValid && r.severity === 'warning'),
        isValid: !Object.values(newFieldResults).some(r => !r.isValid && r.severity === 'error')
      };
    });
  }, []);
  ...
  const accessibleErrors = useMemo((): AccessibleValidationError[] => {
    return getAccessibleValidationErrors(validationState.fieldResults, serviceType);
  ...
  }, [validationState.fieldResults, validationState.touchedFields, config.realTimeValidation, serviceType]);

  // Auto-validate on form data changes (if real-time validation enabled)
  useEffect(() => {
    if (config.realTimeValidation && Object.keys(debouncedFormData).length > 0) {
      validateAllFields();
    }
