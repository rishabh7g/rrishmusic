...
import { scrollToService, isElementInViewport } from '@/utils/smoothScroll'
...
const SERVICES_ORDER: ServiceType[] = [
  'performance',
  'teaching',
  'collaboration',
]
...
export const useServiceNavigation = () => {
  ...
  const navigateNext = useCallback(async (): Promise<void> => {
    if (!navigationState.canNavigateNext || !navigationState.activeService)
      return
  ...
  const navigatePrevious = useCallback(async (): Promise<void> => {
    if (!navigationState.canNavigatePrevious || !navigationState.activeService)
      return
