...
const detectServiceType = (path: string): string | undefined => {
  ...
  if (path.includes('/performance') || path.includes('show')) return 'performance';
  if (path.includes('/collaboration') || path.includes('project')) return 'collaboration';
  return undefined;
};
...
export const useNavigationHistory = (
   ...
   */
  const saveToStorage = useCallback((newHistory: NavigationHistoryEntry[]) => {
    if (opts.persistToStorage && typeof window !== 'undefined') {
      try {
   ...
   */
  const getRecentPages = useCallback((count: number = 5): NavigationHistoryEntry[] => {
    return history
      .slice(-count)
   ...
   */
  const getServiceHistory = useCallback((serviceType: string): NavigationHistoryEntry[] => {
   ...
   */
  useEffect(() => {
    const pathChanged = history.length === 0 || 
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.altKey && event.key === 'ArrowRight' && currentIndex < history.length - 1) {
        event.preventDefault();
        goForward();
      }
    };
