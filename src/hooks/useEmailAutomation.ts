...
export function useEmailAutomation() {
   ...
   */
  const initializeSequence = useCallback(async (formData: ContactFormData): Promise<EmailAutomationResult> => {
    setState(prev => ({ ...prev, isInitializing: true, error: null }));

    try {
      const result = await emailAutomationService.initializeFollowUpSequence(formData);
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to initialize sequence';
   ...
   */
  const cancelSequence = useCallback(async (sequenceId: string, reason?: string): Promise<boolean> => {
    try {
      const success = await emailAutomationService.cancelSequence(sequenceId, reason);
      
      if (!success) {
        setState(prev => ({ ...prev, error: 'Failed to cancel email sequence' }));
      }

      return success;
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Failed to cancel sequence';
  ...
  const previewTemplate = useCallback((
    ...
    return emailAutomationService.previewEmail(serviceType, templateType, sampleData);
  }, []);
...
export function useEmailAutomationDebug() {
  ...
  const loadDebugData = useCallback(() => {
    ...
    try {
      const sequences = JSON.parse(localStorage.getItem('email_sequences') || '[]') as SequenceMetadata[];
      const scheduledEmails = JSON.parse(localStorage.getItem('scheduled_emails') || '[]') as ScheduledEmailData[];
  ...
  const getDebugStats = useCallback(() => {
    return {
      ...
      activeSequences: debugData.sequences.filter(seq => seq.status === 'active').length,
      totalScheduledEmails: debugData.scheduledEmails.length,
      emailsByService: debugData.scheduledEmails.reduce((acc, email) => {
...
export function useContactFormAutomation(serviceType: ServiceType) {
  const { initializeSequence, isInitializing, error, isSuccess, clearError } = useEmailAutomation();

  /**
   * Handle form submission with automatic email sequence initialization
   */
  const handleFormSubmission = useCallback(async (formData: Omit<ContactFormData, 'serviceType'>) => {
    ...
    if (result.success) {
      console.log(`Email sequence initiated for ${serviceType} inquiry from ${formData.name}`);
    }
