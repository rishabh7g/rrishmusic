...
export function useJourneyTracking(
  ...
  const [conversionGoalsAchieved, setConversionGoalsAchieved] = useState<string[]>([]);
  ...
  const startJourney = useCallback((journeyId: string) => {
    const journey = Object.values(JOURNEY_MAPS).find(map => map.id === journeyId);
    if (!journey) {
    ...
    if (enableAnalytics) {
      trackEvent('journey_started', { journeyId, startStep: journey.startStep });
    }
   ...
   */
  const trackConversionGoal = useCallback((goalId: string) => {
  ...
  }, [currentSession, currentJourney, conversionGoalsAchieved, trackEvent, recordTestConversion]);

  /**
   * Track page views and step progression
   */
  const trackPageView = useCallback((path: string, serviceType?: ServiceType) => {
    if (!currentSession || !currentJourney) return;

    // Find matching step in journey
    const matchingStep = currentJourney.steps.find(step => 
      step.path === path || 
      (step.serviceType === serviceType && step.path.includes(serviceType || ''))
    );
      ...
      setCurrentSession(prev => prev ? {
        ...
        metadata: {
          ...
          timeOnPage: {
            ...
            [prev.currentStep]: Date.now() - lastActivityRef.current.getTime()
          }
        }
      } : null);
  ...
  const trackFormComplete = useCallback((formType: string) => {
   ...
   */
  const updateSessionMetadata = useCallback((metadata: Record<string, unknown>) => {
    setCurrentSession(prev => prev ? {
      ...prev,
   ...
   * Exit journey
   */
  const exitJourney = useCallback((exitPoint?: string) => {
    if (!currentSession) return;

    const exitData = {
      exitPoint: exitPoint || currentSession.currentStep,
      completionRate: currentSession.completedSteps.length / (currentJourney?.steps.length || 1),
  ...
  const optimizeJourney = useCallback(() => {
    ...
    const journeyMetrics = JourneyOptimizer.analyzeJourney(currentJourney.id, mockSessions);
    setMetrics(journeyMetrics);

    const recommendations = JourneyOptimizer.generateRecommendations(journeyMetrics, currentJourney);
   ...
   */
  const assignToTest = useCallback((testId: string) => {
  ...
  const recordTestConversion = useCallback((goalId: string) => {
  ...
  useEffect(() => {
    ...
    const initialJourney = determineJourneyFromPath(location.pathname, serviceType);
    if (initialJourney) {
  ...
  }, [location.pathname, serviceType, enableAnalytics, currentSession, trackPageView]);
  ...
  const journeyProgress = useMemo(() => {
    ...
    return (currentSession.completedSteps.length / currentJourney.steps.length) * 100;
...
export function useSimpleJourneyTracking(serviceType: ServiceType) {
  ...
  const trackSimpleEvent = useCallback((eventName: string) => {
    if (!eventsTracked.includes(eventName)) {
      setEventsTracked(prev => [...prev, eventName]);
      console.log(`Simple Journey Event: ${eventName} at ${location.pathname}`);
    }
  }, [eventsTracked, location.pathname]);

  useEffect(() => {
    // Simple step detection based on path
    const step = location.pathname === '/' ? 'homepage' :
                 location.pathname.includes(serviceType) ? `${serviceType}-page` :
                 location.pathname.includes('contact') ? 'contact-form' :
                 'unknown';
 ...
 * Helper functions
 */
function determineJourneyFromPath(path: string, serviceType?: ServiceType): JourneyMap | null {
